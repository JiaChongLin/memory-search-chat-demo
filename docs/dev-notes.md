...
### 0. 前端当前按“左侧导航 + 右侧聊天主区”组织

当前静态前端已经不再使用旧的三栏管理布局，而是改成：

- 左侧导航栏
  负责新聊天、新项目、项目树和未归属会话列表
- 右侧聊天主区
  负责当前会话状态、聊天消息、summary、sources 和调试信息
...
这样可以更直接地观察“项目边界 + 会话可见性”对聊天上下文的影响。
...
- 当前右侧进一步改成聊天主区优先的布局：顶部是会话头部，中间是消息主区，summary / debug / quick test 收进次级信息区，底部输入框固定在聊天区底部。

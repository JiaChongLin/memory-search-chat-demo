...
- 会话只保留两种状态：ctive 和 rchived。rchived 会话仍可查询，但不能继续聊天，也不会进入上下文检索。
- 会话删除改为硬删除。删除会话时，会同步删除该会话下的 ChatMessage 和 SessionSummary。
- 项目删除改为级联硬删除。删除项目时，项目内全部会话、消息和摘要都会一并删除，不会转成未归属会话。
- 当前不提供项目归档。项目管理只保留创建、列表、查看、删除。

## SQLite 兼容性说明

- 这次调整涉及删除语义和外键级联规则。如果本地还在使用旧 SQLite 文件，建议删除旧库后重新初始化，以确保项目删除和会话删除的级联行为与当前规则一致。

## 当前管理规则

- 会话只保留两种状态：`active` 和 `archived`。`archived` 会话仍可查询，但不能继续聊天，也不会进入上下文检索。
- 会话删除改为硬删除。删除会话时，会同步删除该会话下的 `ChatMessage` 和 `SessionSummary`。
- 项目删除改为级联硬删除。删除项目时，项目内全部会话、消息和摘要都会一并删除，不会转成未归属会话。
- 当前不提供项目归档。项目管理只保留创建、列表、查看、删除。
